...
> Container-Orchestrator - Open source tool by AetherCodeHQ

`cli` `containers` `docker` `golang` `open-source` `orchestration`

## What is it?

**Container Orchestrator** is a devops tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Container-Orchestrator --path ./target

# With options
./Container-Orchestrator --path ./target --format json --output report.json

# Verbose mode
./Container-Orchestrator --path ./target --verbose
```
...
### Basic scan
```bash
./Container-Orchestrator --path ./my-project
```

### JSON report
```bash
./Container-Orchestrator --path ./my-project --format json --output report.json
```
...
- name: Run Container Orchestrator
  run: ./Container-Orchestrator --path . --format json --output report.json
```
